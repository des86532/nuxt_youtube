  ...
  div(:class="{'sidebar' : sidebarFixed}")
    .h-full.bg-gray-700.transition-width.duration-300.relative.pb-14.z-10(:class="isSidebarOpen ? 'w-48' : 'w-0'")
      ...
      .w-full.h-14.border-t.text-center.text-white.absolute.bottom-0.flex.justify-center.items-center.cursor-pointer(@click="logout")
        span 登出
    .h-full.flex-1.bg-gray-400.bg-opacity-70.absolute.w-screen(v-if="sidebarFixed && isSidebarOpen" @click="closeSidebar")
</template>
...
export default {
  ...
  computed: {
    ...
    sidebarFixed() {
      const routerList = ['watch', 'search']
      return routerList.includes(this.$route.name)
    }
  },
